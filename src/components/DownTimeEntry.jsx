...
function DownTimesEntry() {
  const [refreshData, setRefreshData] = useState(false)

  useEffect(() => {
    getDownTimeApiCaller()
  }, [refreshData])

  const getDownTimeApiCaller = async () => {
    try {

      const response = await apiGetDownTime();
  ...
  const [downTimeToEdit, setDownTimeToEdit] = useState({})
  ...
  const handleEditDowntime = (row) => {
    setDownTimeToEdit(row)
    setEditModal(true)
  ...
  return (
    <>
      <div style={{ padding: "0px 20px" }}>
                  ...
                  : tableData
                    ...
                    .map((row) => (
                      <StyledTableRow key={row.id}>
                        ...
                        <StyledTableCell className="table-cell">
                          {row.startDownDate} {/* Display Start Time */}
                        </StyledTableCell>

                        <StyledTableCell className="table-cell">
                          {row.endDownDate} {/* Display End Time */}
                        </StyledTableCell>
                        {/* 
                        <StyledTableCell className="table-cell">
        ...
        {addOpen && <AddMachineDownTimeModal setAddOpen={setAddOpen} addOpen={addOpen} setTableData={setTableData} tableData={tableData} downTimeReasons={downTimeReasons} setRefreshData={setRefreshData} />}
